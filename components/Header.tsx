...
const Header: React.FC<HeaderProps> = ({ profile, institutionProfile }) => {
  ...
  return (
    <header className="bg-white shadow-sm p-4 flex justify-between items-center z-10">
      <h1 className="text-2xl font-semibold text-brand-primary truncate" title={institutionProfile?.name || 'EduComply'}>
      ...
      <div className="flex items-center space-x-4">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <p className="font-semibold text-sm text-slate-700">{profile?.full_name || 'Usuario'}</p>
            <p className="text-xs text-slate-500">{profile ? capitalize(profile.role.replace('_', ' de ')) : 'Rol no definido'}</p>
          </div>
          <button 
